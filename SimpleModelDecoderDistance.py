...
class SpatioTemporalModelDistance(nn.Module):
...
def train(root_path, n_iter=500, iter_start=0, mod=0):
    ...
    model = SpatioTemporalModelDistance(dl.nu, dl.nv, dl.nt, sampling_list=dl.sampling_list, vid_coor_rad=dl.vid_coor_rad, vid_pop=dl.vid_pop)
    if iter_start != 0:
        model.load_state_dict(torch.load(root_path + 'model_simple_decoder_nt_' + str(mod) + '_' + str(iter_start) + '.md'))
    ...
    for iter in range(iter_start + 1, n_iter + 1):
        ...
        if iter % 5 == 0:
            torch.save(model.state_dict(), root_path + 'model_simple_decoder_nt_' + str(mod) + '_' + str(iter) + '.md')
    ...
    for iter in range(iter_start, 0, -5):
        ...
        if iter_start != 0:
            model.load_state_dict(
                torch.load(root_path + 'model_simple_decoder_nt_' + str(mod) + '_' + str(iter) + '.md'))
        hits = np.zeros(3)
        cnt = 0
